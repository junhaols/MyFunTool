function CluInfo=LJH_NCluster_vector(ClusteringMat,surfFile,cluster_label,thr)
...
mask=zeros(N,1);
mask(vert_index(find(clustering_label==cluster_label)))=1;
texdata=zeros(N,1);
texdata(vert_index(find(clustering_label==cluster_label)))=cluster_label;
